...
import com.services.domain.{ID, User, UserDescription}
...
private[interpreters] trait UserInterpreters {
  final protected def inMemory[F[_] : Sync]: F[Users[F]] = Ref.of[F, Map[String Refined NonEmpty, User]](Map.empty).map {
    users => new Users[F] {
      ...
      override def create: UserDescription => F[Unit] = {
        case UserDescription(name)=> users.update(_ + (name -> User(ID(name), name)))
      }
